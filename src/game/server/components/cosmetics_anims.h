...
class CLoveAnim : public CCosmeticsHandler::CCosAnim
{
...
public:
	CLoveAnim(vec2 Pos, int64 Tick, CGameMap *pGameMap) : CCosmeticsHandler::CCosAnim(Pos, Tick, pGameMap)
	{
	...
	virtual void Tick()
	{
		...
		for (int i = 0; i < NUM; i++)
		{
			...
			{
				m_Spawned[i] = true;
				GameServer()->CreateSound(GetGameMap(), m_Pos[i], SOUND_PICKUP_HEALTH);
			}
		}
...
class CThunderstorm : public CCosmeticsHandler::CCosAnim
{
...
public:
	CThunderstorm(vec2 Pos, int64 Tick, CGameMap *pGameMap) : CCosmeticsHandler::CCosAnim(Pos, Tick, pGameMap)
	{
	...
	virtual void Tick()
	{
		for (int i = 0; i < NUM; i++)
		{
			if (GetTick() + Server()->TickSpeed() * 3.0f * (i/(float)NUM) <= Server()->Tick() && m_Spawned[i] == false)
			{
				...
				GameServer()->CreateSound(GetGameMap(), m_Pos[i], SOUND_BODY_LAND);
			}
		}
	}
